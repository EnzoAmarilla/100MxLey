...
interface GlobalData  { mode: "global"; connectedClients: number; pendingTotal: number; incidenceTotal: number }
...
export default function AdminDashboard() {
  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? (
      ...
      ) : !activeClient && data?.mode === "global" ? (
        // ── Global view ────────────────────────────────────────────────────────
        <>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <MetricCard label="Clientes conectados"    value={(data as GlobalData).connectedClients} icon={Users}         colorBorder="border-amber-900/40" colorText="text-amber-400" />
            <MetricCard label="Pedidos pendientes"     value={(data as GlobalData).pendingTotal}      icon={Clock}         colorBorder="border-zinc-700/50"  colorText="text-zinc-300" />
            <MetricCard label="Incidencias abiertas"   value={(data as GlobalData).incidenceTotal}    icon={AlertTriangle} colorBorder="border-orange-900/40" colorText="text-orange-400" />
          </div>
        ...
        <>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <MetricCard label="Pendientes"            value={(data as ClientData).pendiente}            icon={Clock}         colorBorder="border-zinc-700/50"   colorText="text-zinc-300" />
            <MetricCard label="En preparación"        value={(data as ClientData).en_preparacion}       icon={Package}       colorBorder="border-amber-900/40"  colorText="text-amber-400" />
            <MetricCard label="Listos para despachar" value={(data as ClientData).listo_para_despachar} icon={Activity}      colorBorder="border-blue-900/40"   colorText="text-blue-400" />
            <MetricCard label="Enviados"              value={(data as ClientData).enviado}              icon={Truck}         colorBorder="border-cyan-900/40"   colorText="text-cyan-400" />
            <MetricCard label="En camino"             value={(data as ClientData).en_camino}            icon={Truck}         colorBorder="border-violet-900/40" colorText="text-violet-400" />
            <MetricCard label="Entregados"            value={(data as ClientData).entregado}            icon={CheckCircle}   colorBorder="border-emerald-900/40" colorText="text-emerald-400" />
            <MetricCard label="Cancelados"            value={(data as ClientData).cancelado}            icon={XCircle}       colorBorder="border-red-900/40"    colorText="text-red-400" />
            <MetricCard label="Incidencias"           value={(data as ClientData).incidencia}           icon={AlertTriangle} colorBorder="border-orange-900/40" colorText="text-orange-400" />
          </div>
          ...
          <div className="rounded-xl border border-zinc-800 bg-white/[0.02]">
            ...
            {(data as ClientData).recentActivity.length === 0 ? (
              <p className="text-center text-zinc-500 text-sm py-8">Sin actividad reciente</p>
            ) : (
              <div className="divide-y divide-zinc-800/50">
                {(data as ClientData).recentActivity.map((act) => (
