...
import { STATUS_LABELS, STATUS_COLORS, OPERATIONAL_STATUSES, type OperationalStatus } from "@/lib/admin-constants";
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between gap-3">
        <div className="min-w-0">
          <h1 className="text-xl sm:text-2xl font-bold text-white truncate">Pedidos / Logística</h1>
          <p className="text-sm text-zinc-400 mt-0.5 truncate">{total} pedidos de {activeClient.name}</p>
        </div>
        <button onClick={fetchOrders} className="flex items-center gap-2 px-3 py-2 rounded-lg bg-white/5 hover:bg-white/10 text-zinc-300 text-sm border border-zinc-700/50 transition-all shrink-0">
      ...
      <div className="flex flex-col sm:flex-row sm:flex-wrap gap-3">
        ...
        <div className="flex items-center gap-1.5 bg-white/5 rounded-lg p-1 border border-zinc-700/50 overflow-x-auto">
          <Filter className="h-3.5 w-3.5 text-zinc-500 ml-1 shrink-0" />
