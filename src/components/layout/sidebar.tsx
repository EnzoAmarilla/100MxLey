...
const navItems = [
  ...
  { label: "Preparación / Picking", href: "/picking",            icon: ScanLine, logisticsGated: true },
...
export function Sidebar() {
  ...
  return (
    <>
      {/* Mobile hamburger button — overlays the header area */}
      <button
        onClick={() => setMobileOpen(true)}
        className="fixed top-0 left-0 z-50 h-14 w-14 flex items-center justify-center lg:hidden text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors"
      ...
      {/* Sidebar panel */}
      <aside
        className={`fixed left-0 top-0 z-40 h-screen w-64 flex flex-col border-r border-brand-border bg-brand-bg transition-transform duration-300 ease-in-out ${
      ...
      >
        ...
        <div className="relative flex items-center gap-3 px-5 py-5 border-b border-brand-border">
          ...
          <button
            onClick={closeMobile}
            className="lg:hidden p-1.5 rounded-lg text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-brand-surface transition-colors"
